...
export const FormularioPage: React.FC<FormularioPageProps> = ({ onNavigateHome }) => {
  ...
  const validate = () => {
    ...
    if (formData.hasCurrentPlan === 'Sim' && !formData.currentCarrier?.trim()) {
      newErrors.currentCarrier = 'Informe a operadora atual da sua empresa.';
    }
  ...
  const getWhatsAppLeadMessage = () => {
    ...
    msg += `🏢 *Empresa:* ${formData.companyName || 'Não informada'} | *CNPJ:* ${formData.hasCnpj}\n`;
    msg += `👥 *Quem será incluído:* ${formData.includedProfiles.join(', ')}\n`;
    msg += `🎂 *Idades:* ${formData.agesList}\n`;
    msg += `📋 *Possui plano atual:* ${formData.hasCurrentPlan}${formData.hasCurrentPlan === 'Sim' ? ` (${formData.currentCarrier || ''} - ${formData.currentMonthlyCost || 'Valor n/i'})` : ''}\n`;
    msg += `🎯 *Objetivo:* ${formData.mainGoal}\n`;
            ...
            <div className="bg-white rounded-3xl p-8 sm:p-12 text-[#07172E] shadow-2xl text-center space-y-6 border border-slate-200 animate-fadeIn">
              ...
              <div className="space-y-2">
                ...
                <p className="text-slate-600 text-sm sm:text-base max-w-lg mx-auto">
                  Nossos consultores da <strong>Ultra Seguros</strong> já estão analisando as melhores tabelas para o perfil da sua empresa e entrarão em contato via <strong>{formData.contactPreference}</strong>.
                </p>
              </div>
              ...
              <div className="bg-slate-50 rounded-2xl p-5 border border-slate-200 text-left text-xs sm:text-sm text-slate-700 space-y-2 max-w-md mx-auto">
                <div className="flex justify-between py-1 border-b border-slate-200/60">
                  ...
                  <span className="font-semibold text-slate-900">{formData.hasCnpj}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-slate-200/60">
              ...
              >
                ...
                <div className={`space-y-3 pb-6 border-b border-slate-200 ${errors.hasCnpj ? 'form-error-marker' : ''}`}>
                  ...
                  <p className="text-xs text-slate-500">
                    Planos com CNPJ têm condições e valores até 40% mais acessíveis que planos individuais.
                  </p>

                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
                    {(['Sim', 'Sou MEI', 'Não'] as const).map((option) => {
                      const isSelected = formData.hasCnpj === option;
                      return (
                        <label
                          key={option}
                          onClick={() => {
                            setFormData((prev) => ({ ...prev, hasCnpj: option }));
                ...
                <div className={`space-y-3 pb-6 border-b border-slate-200 ${errors.agesList ? 'form-error-marker' : ''}`}>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[11px] text-slate-400 italic">
                      Dica: você pode digitar as idades separadas por vírgula ou listar o total por faixa (ex: "2 pessoas com 35, 1 com 40").
                    </p>
                  </div>
                  {errors.agesList && (
                    <p className="text-xs text-[#E52B32] font-semibold flex items-center gap-1 mt-1">
                  ...
                  {formData.hasCurrentPlan === 'Sim' && (
                    <div className="p-4 sm:p-5 rounded-2xl bg-slate-50 border border-slate-200/90 space-y-4 animate-fadeIn">
                      <div className="text-xs font-bold text-[#07172E] uppercase tracking-wider flex items-center gap-1.5">
                        ...
                        <span>Detalhes do Plano Atual</span>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          ...
                          <input
                            type="text"
                            placeholder="Ex: Bradesco, Unimed, Amil, SulAmérica..."
                        ...
                        <div className="space-y-1.5">
                          <label className="block text-xs font-bold text-slate-700">
                            Qual o valor aproximado da mensalidade total do plano?
                          </label>
                          <input
                            type="text"
                            placeholder="R$ 0,00"
                ...
                <div className="space-y-4 pt-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      <label className="block text-xs font-bold text-slate-700">
                        E-mail para envio da proposta <span className="text-[#E52B32]">*</span>
                      </label>
                      <div className="relative">
                    ...
                    <div className="space-y-1.5">
                      <label className="block text-xs font-bold text-slate-700">
                        Nome da Empresa / Cidade-UF <span className="text-slate-400 font-normal">(Opcional)</span>
                      </label>
                      <div className="relative">
                        <Building2 className="w-4 h-4 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" />
                        <input
                          type="text"
                          placeholder="Ex: Empresa ABC - São Paulo/SP"
                ...
                <div className="pt-2">
                  <label className="flex items-start gap-2.5 cursor-pointer text-xs text-slate-600 select-none">
                    ...
                    />
                    <span>
                      Concordo em receber o contato da <strong>Ultra Seguros Corretora</strong> para elaboração e apresentação desta cotação, conforme a LGPD.
                    </span>
                  </label>
                  {errors.lgpdAccepted && (
